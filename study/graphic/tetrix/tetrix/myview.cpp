...
void MyView::clearFullRows()
{
    for(int y = 429; y > 50; y-=20)  {
        QList<QGraphicsItem *> list = scene()->items(199, y, 202,
                                                     22, Qt::ContainsItemShape, Qt::AscendingOrder);
        if(list.count() == 10) {
...
void MyView::restartGame()
{
    ...
    foreach (QGraphicsItem *item,
             scene()->items(199, 49, 202, 402, Qt::ContainsItemShape, Qt::AscendingOrder)) {
...
void MyView::initView()
{
    ...
    mask->setPalette(QPalette(QColor(0, 0, 0, 88)));
    mask->resize(900, 600);
    ...
    option->setPalette(QPalette(QColor(0, 0, 0, 188)));
    ...
    QPushButton *helpCloseButton = new QPushButton(tr("帮    助"), option);
    palette.setColor(QPalette::ButtonText, Qt::black);
    ...
    help->setPalette(QPalette(QColor(0, 0, 0, 188)));
    help->resize(300, 400);
    QGraphicsWidget *helpWidget = scene->addWidget(option);
    helpWidget->setPos(250, 50);
    helpWidget->setZValue(3);
    helpWidget->hide();

    QLabel *helpLabel = new QLabel(help);
    helpLabel->setText(tr("<h1><font color=white>xiao</font></h1>"));
    helpLabel->setAlignment(Qt::AlignCenter);
    helpLabel->move(30, 150);

    gameWelcomeText = new QGraphicsTextItem(0);
    scene->addItem(gameWelcomeText);
    gameWelcomeText->setHtml(tr("<font color=white>方块游戏</font>"));
    ...
    gamePauseText->setHtml(tr("<font color=white>游戏暂停</font>"));
...
void MyView::initGame()
{
    ...
    gameLevelText->setHtml(tr("<font color=white>第<br>一<br>幕<br></font>"));
...
void MyView::updateScore(const int fullRowNum)
{
    ...
    if(currentScore < 500) {
        ...
        gameLevelText->setHtml(tr("<font color=white>第<br>二<br>幕<br></font>"));
